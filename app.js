...
game.catchPokemonn = (pokemonObj)=>{
  game.party.push(pokemonObj);
  game.items[2].quantity--;
};

game.catchPokemonn(pokemon[9]);
console.log(game.party);
...
game.gymStatus=()=>{
  game.gyms.forEach((gym)=>{
    if(gym.completed){
      gymTally.completeGyms++
    }
   if(!gym.completed){
      gymTally.incompleteGyms++
    }
  });
}
game.gymStatus();
console.log("Completed gyms:"  + gymTally.completeGyms +  " " + "Incompleted gyms: " + gymTally.incompleteGyms);
...
/*
I commented those because they take a wide space in the output
